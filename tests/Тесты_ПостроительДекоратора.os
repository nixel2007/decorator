...
Процедура ДекораторСоздаетсяПоТипуСПараметризованнымКонструктором() Экспорт
	...
	Декоратор = Новый ПостроительДекоратора(ПроверяемыйТип)
		.ДобавитьЗначениеПараметраКонструктораПоТипу(1)
		.Построить();
	...
	Декоратор = Новый ПостроительДекоратора(ПроверяемыйТип)
		.ДобавитьЗначениеПараметраКонструктораПоТипу(1)
		.ДобавитьЗначениеПараметраКонструктораПоТипу(3)
		.Построить();
...
Процедура ФункцияСПараметрамиДобавляется() Экспорт
	...
	НовыйМетод = Новый Метод("ДобавленнаяФункция")
		.Публичный()
		.ДобавитьПараметр(Новый ПараметрМетода("Параметр1"))
		.ДобавитьПараметр(Новый ПараметрМетода("Параметр2").ЗначениеПоУмолчанию(2))
...
Процедура ПерехватчикиДобавленныхФункцийРаботаютКорректно() Экспорт
	...
	ПерехватчикПосле = Новый Перехватчик(ИмяМетода)
		...
		.Тело("КэшПоля = КэшПоля - 1;");

	Декоратор = Новый ПостроительДекоратора(МойКласс)
		.ДобавитьПоле(Новый Поле("КэшПоля").Публичное())
		.ДобавитьПерехватчик(ПерехватчикПеред)
		.ДобавитьМетод(НовыйМетод)
		.ДобавитьПерехватчик(ПерехватчикПосле)
		.Построить();
...
Процедура АннотированноеПолеДобавляется() Экспорт
	...
	Поле = Новый Поле("ПубличноеПоле")
		.Публичное()
		.ДобавитьАннотацию(Новый Аннотация("АннотацияБезПараметра"))
		.ДобавитьАннотацию(
			Новый Аннотация("АннотацияСПараметром")
				.ДобавитьПараметр("Значение"))
		.ДобавитьАннотацию(
			Новый Аннотация("АннотацияСИменованнымПараметром")
				.ДобавитьПараметр("Значение", "ИмяПараметра"));
...
Процедура ИмпортДобавляется() Экспорт
	...
	ТекстСценарияДекоратора = Новый ПостроительДекоратора(Новый МойКласс)
		.ДобавитьИмпорт(Новый Импорт("asserts"))
		.ДобавитьИмпорт(Новый Импорт(".").ТипПодключения(ТипыПодключенияБибилотек.ПоПути))
		.ТекстСценария();
...
Процедура ПользовательскийШагДобавляется() Экспорт
	...
	ТекстСценарияДекоратора = Новый ПостроительДекоратора(Новый МойКласс)
		.ДобавитьШагИнициализации(Новый ШагИнициализации("А = 0;"))
		.ДобавитьШагИнициализации(Новый ШагИнициализации("Сообщить(А);"))
		.ТекстСценария();
...
Процедура ПереопределениеТекстаСценария() Экспорт
	...
	ПостроительДекоратора = Новый ПостроительДекоратора(Новый МойКласс())
		.ДобавитьПоле(Новый Поле("МоеПоле").Публичное());
...
Процедура НекорректныеПараметрыАннотации() Экспорт
	...
	Попытка
		Аннотация.ДобавитьПараметр("ЗначениеЛомающееАннотацию")
	Исключение
		ИсключениеВозникло = Истина;
	КонецПопытки;
