...
// Внешний -> КакойТоКласс, без промежуточного вызова через Внутренний
Ожидаем.Что(Внешний.СчётчикВызовов).Равно(0);
```
...
выбрасывает исключение с подсказкой использовать `ЭтотОбъект.<метод>`. Проверка
ищет обращение по тексту, поэтому не увидит вызов через промежуточную переменную
и может сработать на упоминание в строке или комментарии.
...
Объединение отключается и во всём процессе — установкой любого значения
переменной окружения ```OSCRIPT_DECORATOR_NO_MERGE_LAYERS```.
...
//
//  Возвращаемое значение:
//   Произвольный - Обёрнутый объект. Неопределено, если объект не является декоратором
//                  или декоратор построен без исходного объекта
//
...
либо переменной окружения `OSCRIPT_DECORATOR_NO_MERGE_LAYERS` для всего процесса.
...
Исправлено: методы с зарезервированными именами (`Найти`, `ПриСозданииОбъекта` и их
английские синонимы) объявляются в декораторе под именем `_<Имя>`, но делегируются
исходному объекту под настоящим именем. Раньше делегирование шло по переименованному
имени, поэтому вызов такого метода у декоратора падал с "Метод объекта не обнаружен".
